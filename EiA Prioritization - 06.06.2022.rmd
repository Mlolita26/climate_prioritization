...
FileName<-"Data/hazard_vop_admin.parquet"
...
if(!file.exists(FileName)){
  ...
  Data<-data.table(arrow::read_parquet(FileName))
}
...
dff<-reactive({
   Data[Label %in% input$Crops.Lab
        ][,VoP:=round(VoP,0)][,Unit:=paste("2015",input$Units)]
})
...
square_val<-reactive({
  ...
  dat[,VoP:=VoP/max_vop]
  #data[,VoP:=round(Scaler*VoP/max_vop,0)]
...
One square = `r reactive(square_val())` million I$ 2015.
...
Waffler<-function(data,
  ...
  max_vop<-10^(nchar(data[,max(VoP)])-2)
  data[,VoP:=VoP/max_vop]
  scale_factor<-data[,max(VoP,na.rm = T)/Scaler]
  data[,VoP:=round(VoP/scale_factor,0)]
  ...
  MaxY<-data[,list(Height=ceiling(sum(VoP)/NRows)),by=.(Region,Country,Crop)][,max(Height)]+1
  ...
  Plots<-lapply(data[,sort(unique(Admin))],FUN=function(REG){
    
    Plot.Dat<-data[Admin==REG]
...
PlotData<-reactive({
  Waffler(data=data.table::copy(dff()),
          Scaler=input$Scaler,
          unit_lab="I$ 2015",
          NRows=input$NRows,
...
Plot.Height <- function(){
  input$Plot.Height*400*length(PlotData())
}
...
renderPlot({
  ggpubr::ggarrange(plotlist=PlotData(), 
                    ncol=input$GridCols,
                    nrow=input$GridRows, 
                    common.legend = TRUE, 
                    legend=input$LegPos)
...
renderDT({
  datatable(dff()[,.(Crop,Region)],
            caption="Plot data by crop x region",
            extensions = 'Buttons',
            filter = "top",
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
